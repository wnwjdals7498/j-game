...
|---|---|---|
| 총 단어 수 | 16 | **미측정 — 승인 대기** |
| 파일 용량 | 0.1 MB (≤ 10 MB) | 미측정 |
| 뜻풀이 최대 길이 | 31 (≤ 80) | 미측정 |
| 빈도 매칭 | 14 (88%) | 미측정 |
| 고유 음절 수 | 43 (한 번만 등장 42) | 미측정 |

**음절 커버리지가 이번 리포트의 진짜 산출물**이다. 43개 음절 중 42개가 단 한 번만
등장한다 — 16표제어짜리 샘플에서는 당연한 결과지만, 실데이터에서도 이 값이 높으면
...
검사다. 샘플 16행에서는 한 행이 6.25%p 라 티어 비율이 산술적으로 ±2%p 안에 들어갈 수
없고, 단어 수는 16 이라 정의상 실패한다. 산식이나 `bounds` 의 문제가 아니다
(위 02-08 절 "티어 비율 ±1% 는 16행에서 성립하지 않는다" 와 같은 사정이다).
...
으로 떨어진다) 샘플 빌드에서는 건너뛴다. 건너뛰는 조건은 DB 안
...
두 개다. 실데이터가 도착하면 `--fixtures` 없이 한 번 돌려 위 표의 둘째 열을 채운다.
...
- **눈 검수 6항목** — `docs/reports/02-db-report.md` "눈 검수 결과 (사람이 작성)" 가 비어 있다.
  에이전트가 채우면 안 되는 칸이다 (02-10 담당: 판단은 HUMAN). 지금 리포트는 샘플
  16표제어라 난이도 판단의 근거가 못 되므로, **실데이터 도착 후 다시 뽑아서** 검수한다.
- **`docs/LICENSES.md`** — 00-01(이용 신청)의 산출물이라 아직 없다. 생기면 재빌드만으로
