...
app/lib/domain/fixtures/
└─ dummy_dictionary.dart      합성 사전 (알고리즘 검증용)
app/test/domain/fixtures/
└─ sample_korean_words.dart   실제 한국어 200단어 (눈 검수용)
```

`dummy_dictionary.dart` 는 이 문서에서 `test/domain/fixtures/` 에 두었다가
01-10에서 `lib/domain/fixtures/` 로 옮겼다. `tool/measure.dart` 하네스와 테스트가
같이 써야 하기 때문이다. `lib/` 안이므로 `domain/` 규칙(플랫폼 import 금지)을 그대로 지킨다.
...
// 01-10에서 lib/domain/fixtures/ 로 옮겼다.
