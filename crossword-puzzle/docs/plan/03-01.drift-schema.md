...
`puzzle_log` (첫 제출 판정용, `word_stat`과 같은 이유로 갱신 시 보존 대상)는 이 문서
작성 시점에는 없었고, 03-04에서 `tools/schema.sql`에 추가하면서 여기 `app_database.dart`에도
같은 패턴(`@DataClassName`, `.named()`, `tableName` 오버라이드, `@DriftDatabase(tables: [...])`
등록)으로 `PuzzleLogs` 테이블 클래스를 더했다. 위 코드 블록은 03-01 작성 시점 스냅샷이라
`puzzle_log`를 담고 있지 않다 — 실제 정본은 `app/lib/data/db/app_database.dart`다.
