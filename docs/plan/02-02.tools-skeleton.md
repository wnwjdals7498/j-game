...
- [ ] `python -m tools --help` 가 build/report/check 표시
- [ ] `config.py` 에 위 설정값 전부 존재, import 시 `TIER_RATIO` 검증
- [ ] `io_util` JSONL 왕복 동작, 한글이 이스케이프되지 않음
- [ ] `pipeline.STEPS` 에 8단계 등록, 각 모듈이 `run(use_fixtures)` 시그니처
